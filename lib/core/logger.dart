...
class AppLogger {
  ...
  void info(String msg) => _write(LogLevel.info, msg);
  void debug(String msg) => _write(LogLevel.debug, msg);
  void error(String msg) => _write(LogLevel.error, msg);

  // 新增：可监听的日志列表（供 UI 使用）
  final ValueNotifier<List<String>> entries = ValueNotifier<List<String>>([]);

  void _write(LogLevel level, String msg) {
    if (!enabled) return;
    final ts = DateTime.now().toIso8601String();
    final line = '[$ts][${level.name}] $msg';
